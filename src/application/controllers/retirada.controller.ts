...
export default class RetiradaController {
  ...
  async countRetiradas(
    request: Request,
    response: Response,
    next: NextFunction
  ) {
    try {
      const show = await this.retiradaService.countRetiradas();
      return response.status(status.SUCCESS).json(show);
    } catch (error) {
      console.log(error);
      next(error);
  ...
  async countRetiradasByAssistidoId(
    request: Request,
    response: Response,
    next: NextFunction
  ) {
  ...
  async selectRetiradasByDataIntervalo(
    ...
    try {
      ...
      const show = await this.retiradaService.selectRetiradaByDataIntervalo(
        dataInicial,
        dataFinal
      );
      return response.status(status.SUCCESS).json(show);
